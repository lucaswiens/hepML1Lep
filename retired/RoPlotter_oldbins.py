...
from plotClass.plotting.plotGroups import All_files,cut_strings,varList,oldbins
...
lumi = '35.9'#'59.74'#'41.9'#
indir = '/nfs/dust/cms/user/amohamed/susy-desy/ML/hepML_1Lep/root_FRs_w_score_new/'#'/nfs/dust/cms/user/amohamed/susy-desy/CMGSamples/Samples_March19/2018Data/FR_2018Data_/'#'/nfs/dust/cms/user/amohamed/susy-desy/CMGSamples/Samples_March19/2017trees_Mar19/17FR_April15_/'
#'2016_event_CatSig_dphi_nT'#'CRPlots_2018'#'./testplots_CatWJ_testscalontopofalphabetagamma'
outdire = 'SUS_WS19/oldbins'
scale_bkgd_toData = False
do_alphabetagamma = True
do_Oldbins = True
if do_Oldbins :
    varList = []
    varList.append(["LTOldBins","LTOldBins","LTOldBins",[0,0.0,0.0],"LogY",["MoreY",1000]])

alpha,beta,gamma =  0.84 ,1.01 , 0.72
...
if __name__ == '__main__':
    ...
    for g in instPlot.group:
        ...
        if do_Oldbins : 
            ...
            for bin_ in oldbins.keys() : 
                All_files[g][bin_] = instPlot.makecuts(chain,oldbins[bin_])
                #All_files[g]['hist_bins'].SetBinContent(bincount,All_files[g][bin_].GetEntries())
                #bincount+=1
    ...
    outroot = ROOT.TFile(outdire+"/plots_{0}_{1}_{2}".format(currentDT.year,currentDT.month,currentDT.day)+".root","recreate")
        ...
        for key in All_files :
            hist = All_files[key]['hist_bins']
            # draw the variable to the hist created 
            if 'Data' in key : lum = '1.0' 
            else  : lum = lumi
            All_files['1 >> '+hist = All_files[key]['hist_bins'], All_files[key]['scale']+'*'+lum+'*(1)',"goff")
            #print (hist)
