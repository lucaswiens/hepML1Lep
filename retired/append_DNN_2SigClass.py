...
def Predict_Keras(infile,outdir,var_list,class_list,model = None) : 
    ...
    SignLDM_val = array.array('f', [0.])
    SignHDM_val = array.array('f', [0.])
    ...
    if ('T5qqqq' in infile) : 
        ...
        prediction = pd.DataFrame(model.predict_proba(p_df[var_list].values),columns=['TTS', 'TTDi', 'WJ','Sig_LDM','Sig_HDM'])
    ...
    Sign_LDM  = tree_out.Branch('sigLDM', SignLDM_val, 'sigLDM/F')
    Sign_HDM  = tree_out.Branch('sigHDM', SignHDM_val, 'sigHDM/F')
    ...
    for i_ev in range(tree_out.GetEntries()):
        ...
        SignLDM_val[0] = prediction['Sig_LDM'][i_ev]
        SignHDM_val[0] = prediction['Sig_HDM'][i_ev]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exec', help="wight directory", default='./batch/append_exec2Sig.sh', metavar='exec')
    ...
    if not args.batchMode and args.infile: 
        ...
        if ( "T1tttt" in args.infile or 'T5qqqq' in args.infile) :
            Predict_Keras(args.infile,outdir,var_list,['TTS','TTDi','WJ','sig_LDM','sig_HDM'],model = args.model)
        else : 
            Predict_Keras(args.infile,outdir,var_list,['TTS','TTDi','WJ','sig_LDM','sig_HDM'],model = args.model)
    else : 
