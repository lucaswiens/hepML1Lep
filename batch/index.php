...
<h1 class = "header"> 
<div align="center">Ashraf Mohamed's webpage</div>         
</h1>

<h1><div align="center"><?php echo getcwd(); ?></h1></div>
<?php
$has_subs = false;
...
if ($has_subs) {
    ...
    foreach (glob("*") as $filename) {
        if (is_dir($filename) && ($_SERVER['PHP_AUTH_USER'] == 'amohamed' || !preg_match("/^\..*|.*private.*/", $filename))) {
...
if ($_GET['noplots']) {
    ...
    $filenames = glob("png/*.png"); sort($filenames);
    foreach ($filenames as $filename) {
        ...
        foreach ($other_exts as $ex) {
            ...
            $subdirname = str_replace(".","",$ex)."/" ;
            $other_filename = $subdirname . str_replace('png/', '', $other_filename);
